...
import { ArrowLeft, Package, Trash2, TrendingDown, ShoppingCart } from "lucide-react";
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-5 px-4">
        ...
        <div className="text-center space-y-1">
          <h2 className="font-display font-bold text-xl tracking-tight">Your cart is empty</h2>
          <p className="text-sm text-muted-foreground">Ask the agent to find products for you</p>
        </div>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <ScrollArea className="flex-1">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 pb-20 relative z-10">
          {/* Back */}
          <button
            onClick={() => navigate("/")}
          ...
          <h1 className="font-display text-2xl sm:text-3xl font-bold tracking-tight mb-8">Shopping Cart</h1>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
                ...
                {retailerEntries.map(([retailer, items]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="border-t border-border/30 px-5 py-3 bg-secondary/10">
            ...
            <div className="lg:w-[360px] shrink-0">
              <div className="glass-card rounded-2xl p-6 space-y-5 lg:sticky lg:top-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-display font-bold text-xl text-primary">{cs}{subtotal.toFixed(2)}</span>
                  </div>
                </div>
                ...
                {/* Savings pill */}
                <div className="rounded-2xl p-3.5 bg-success/10 border border-success/20">
...
const CartItemRow = ({
  ...
  return (
    <div className="flex items-center gap-4 p-3 rounded-xl bg-secondary/20 hover:bg-secondary/30 transition-colors">
      <div
        className="w-14 h-14 rounded-xl flex items-center justify-center shrink-0"
        style={{ backgroundColor: `${retailerColor}10` }}
      >
        <Package className="h-6 w-6" style={{ color: retailerColor }} />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold truncate">{product.name}</p>
        <p className="text-xs text-muted-foreground mt-0.5">🚚 {product.deliveryEstimate} delivery</p>
      </div>
      <div className="text-right shrink-0">
        <p className="font-display font-bold text-base text-success">{currencySymbol}{product.price.toFixed(2)}</p>
        <p className="text-xs text-muted-foreground line-through">{currencySymbol}{regularPrice.toFixed(2)}</p>
      </div>
    </div>
  );
};
